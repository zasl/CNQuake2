...
> ⚠️ **本项目已停止开发与维护**
>
> 由于 API 变动频繁，且目前没有稳定可用的 API，服务非常不稳定：
> - Wolfx API 对国内网络优化不佳，延迟较大
> - FAN Studio API 已停止服务
>
> 因此本项目不再提供可用服务，仅作为代码归档保留。
...
本应用需在 **HTTPS** 环境下运行，并正确配置 SSL 证书。受腾讯地图 CORS 跨域策略限制，请勿以 `file://` 协议直接打开，否则地图图标将回退为默认 Marker 样式。
...
> [!NOTE]
> 由于项目已停止开发，以下功能将不再实现，仅作为历史规划记录保留。
...
本项目曾使用以下数据源，但由于服务变动均已不可用：

- ~~**Wolfx 防灾 API**~~：对国内网络优化不佳，延迟较大，服务不稳定。
- ~~**FAN Studio - API**（`https://api.fanstudio.tech/`）~~：已于 26H1 停止服务。

由于上述数据源均不再可用，且目前没有稳定可用的替代 API，项目无法继续提供实时预警服务，故停止开发与维护。
