...
> ⚠️ **本專案已停止開發與維護**
>
> 由於 API 變動頻繁，且目前沒有穩定可用的 API，服務非常不穩定：
> - Wolfx API 對國內網路優化不佳，延遲較大
> - FAN Studio API 已停止服務
>
> 因此本專案不再提供可用服務，僅作為程式碼歸檔保留。
...
> [!NOTE]
> 由於專案已停止開發，以下功能將不再實現，僅作為歷史規劃記錄保留。
...
本專案曾使用以下資料來源，但由於服務變動均已不可用：

- ~~**Wolfx 防災 API**~~：對國內網路優化不佳，延遲較大，服務不穩定。
- ~~**FAN Studio - API**（`https://api.fanstudio.tech/`）~~：已於 26H1 停止服務。

由於上述資料來源均不再可用，且目前沒有穩定可用的替代 API，專案無法繼續提供即時預警服務，故停止開發與維護。
